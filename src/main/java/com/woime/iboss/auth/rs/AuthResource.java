package com.woime.iboss.auth.rs;
...
import com.woime.iboss.auth.persistence.manager.RoleManager;
...
public class AuthResource
{
	...
	public UserDTO getUserById(@QueryParam("userId") String userId)
	{
		...
		try
		{
			com.woime.iboss.api.user.UserDTO apiUserDto = userConnector.findById(userId);
			...
			if (apiUserDto == null)
			{
				...
				userDto.setAuthorities(Collections.<String> emptyList());
				userDto.setAttributes(Collections.<String> emptyList());
	...
	public UserDTO getUser(@QueryParam("username") String username)
	{
		...
		try
		{
			com.woime.iboss.api.user.UserDTO apiUserDto = userConnector.findByUsername(username, tenantHolder.getUserRepoRef());
	...
	public UserDTO getUserByUsername(@QueryParam("username") String username, @QueryParam("appId") Long appId)
	{
		...
		com.woime.iboss.api.user.UserDTO apiUserDto = userConnector.findByUsername(username, Long.toString(globalId));
