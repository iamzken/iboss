...
import com.woime.iboss.bpm.support.DelegateTaskHolder;
...
public class HumanTaskTaskListener extends DefaultTaskListener
{
	...
	{
		HumanTaskDTO humanTaskDto = new HumanTaskBuilder().setDelegateTask(delegateTask).setVote(this.isVote(delegateTask)).build();
	...
	{
		List<BpmConfUser> bpmConfUsers = bpmConfUserManager.find("from BpmConfUser where bpmConfNode.bpmConfBase.processDefinitionId=? and bpmConfNode.code=?", delegateTask.getProcessDefinitionId(),
