...
import com.woime.iboss.bpm.cmd.DeleteTaskWithCommentCmd;
import com.woime.iboss.bpm.cmd.FindTaskDefinitionsCmd;
import com.woime.iboss.bpm.cmd.RollbackTaskCmd;
import com.woime.iboss.bpm.cmd.SignalStartEventCmd;
import com.woime.iboss.bpm.cmd.WithdrawTaskCmd;
...
import com.woime.iboss.bpm.persistence.manager.BpmConfOperationManager;
import com.woime.iboss.bpm.persistence.manager.BpmConfUserManager;
import com.woime.iboss.bpm.persistence.manager.BpmTaskConfManager;
...
public class ActivitiInternalProcessConnector implements InternalProcessConnector
{
	...
	public FormDTO findTaskForm(String taskId)
	{
		...
		List<BpmConfOperation> bpmConfOperations = bpmConfOperationManager.find("from BpmConfOperation where bpmConfNode.bpmConfBase.processDefinitionId=? and bpmConfNode.code=?", processDefinitionId,
				activityId);
	...
	public ProcessTaskDefinition findTaskDefinition(String processDefinitionId, String taskDefinitionKey, String businessKey)
	{
		...
		try
		{
			...
			{
				logger.info("cannot find BpmTaskConf {} {}", businessKey, taskDefinitionKey);
			}
		}
		catch (Exception ex)
		{
	...
	{
		GetDeploymentProcessDefinitionCmd getDeploymentProcessDefinitionCmd = new GetDeploymentProcessDefinitionCmd(processDefinitionId);
		ProcessDefinitionEntity processDefinitionEntity = processEngine.getManagementService().executeCommand(getDeploymentProcessDefinitionCmd);
