package com.woime.iboss.bpm.support;
...
import com.woime.iboss.api.process.ProcessDTO;
import com.woime.iboss.api.user.UserConnector;
import com.woime.iboss.bpm.cmd.FindFirstTaskFormCmd;
import com.woime.iboss.bpm.persistence.domain.BpmConfForm;
import com.woime.iboss.bpm.persistence.domain.BpmProcess;
import com.woime.iboss.bpm.persistence.manager.BpmConfFormManager;
...
import com.woime.iboss.spi.humantask.TaskUserDTO;
import com.woime.iboss.spi.process.FirstTaskForm;
...
public class ProcessConnectorImpl implements ProcessConnector
{
	...
	public String startProcess(String userId, String businessKey, String processDefinitionId, Map<String, Object> processParameters)
	{
		...
		/*
		 * // {流程标题:title}-{发起人:startUser}-{发起时间:startTime} String
		 * processDefinitionName =
		 * processEngine.getRepositoryService() .createProcessDefinitionQuery()
		 * .processDefinitionId(processDefinitionId).singleResult() .getName();
		 * String processInstanceName =
		 * processDefinitionName + "-" +
		 * userConnector.findById(userId).getDisplayName() + "-" + new
	...
	public FormDTO findStartForm(String processDefinitionId)
	{
		...
		{
			// 再从数据库里找一遍配置
			com.woime.iboss.spi.humantask.FormDTO humantaskFormDto = taskDefinitionConnector.findForm(firstTaskForm.getActivityId(), processDefinitionId);
		...
		com.woime.iboss.spi.humantask.FormDTO taskFormDto = taskDefinitionConnector.findForm(firstTaskForm.getActivityId(), firstTaskForm.getProcessDefinitionId());

		List<BpmConfForm> bpmConfForms = bpmConfFormManager.find("from BpmConfForm where bpmConfNode.bpmConfBase.processDefinitionId=? and bpmConfNode.code=?", firstTaskForm.getProcessDefinitionId(),
	...
	public Page findCompletedProcessInstances(String userId, String tenantId, Page page)
	{
		...
		List<HistoricProcessInstance> historicProcessInstances = historyService.createHistoricProcessInstanceQuery().startedBy(userId).processInstanceTenantId(tenantId).finished()
	...
	public Page findInvolvedProcessInstances(String userId, String tenantId, Page page)
	{
		...
		List<HistoricProcessInstance> historicProcessInstances = historyService.createHistoricProcessInstanceQuery().processInstanceTenantId(tenantId).involvedUser(userId)
				.listPage((int) page.getStart(), page.getPageSize());
	...
	public Page findHistoryTasks(String userId, String tenantId, Page page)
	{
		...
		List<HistoricTaskInstance> historicTaskInstances = historyService.createHistoricTaskInstanceQuery().taskTenantId(tenantId).taskAssignee(userId).finished().listPage((int) page.getStart(),
				page.getPageSize());
	...
	public Page findCandidateOrAssignedTasks(String userId, String tenantId, Page page)
	{
		...
		List<Task> tasks = taskService.createTaskQuery().taskTenantId(tenantId).taskCandidateOrAssigned(userId).listPage((int) page.getStart(), page.getPageSize());
		page.setResult(tasks);
		page.setTotalCount(count);
	...
	public Page findHistoricProcessInstances(String tenantId, Page page)
	{
		...
		List<HistoricProcessInstance> historicProcessInstances = historyService.createHistoricProcessInstanceQuery().processInstanceTenantId(tenantId).listPage((int) page.getStart(),
				page.getPageSize());
		page.setResult(historicProcessInstances);
		page.setTotalCount(count);
