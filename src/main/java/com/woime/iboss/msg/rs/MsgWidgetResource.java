...
{
	private static Logger logger = LoggerFactory.getLogger(MsgWidgetResource.class);
	...
	public String msg() throws Exception
	{
		...
		List<MsgInfo> msgInfos = (List<MsgInfo>) msgInfoManager.pagedQuery(hql, 1, 10, userId).getResult();
		...
		{
			buff.append("    <tr>");
			buff.append("      <td>" + this.findDisplayName(msgInfo.getSenderId()) + "</td>");
			buff.append("      <td>" + dateFormat.format(msgInfo.getCreateTime()) + "</td>");
			buff.append("      <td>");
			buff.append("        <a href='" + ".." + "/msg/msg-info-view.do?id=" + msgInfo.getId() + "'>" + this.substr(msgInfo.getContent()) + "</a>");
			buff.append("      </td>");
			buff.append("    </tr>");
		}
