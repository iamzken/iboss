...
import com.woime.iboss.security.SecurityConstants;
...
public class RememberLastUsernameAuthenticationSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler
{
	...
	public void handleUsername(HttpServletResponse response, Authentication authentication)
	{
		String username = this.getUsername(authentication);
		this.addCookie(response, SecurityConstants.SECURITY_LAST_USERNAME, username);
	}

	public void handleDevice(HttpServletRequest request, HttpServletResponse response)
	{
		...
		if (deviceId == null)
		{
			...
			this.addCookie(response, "SECURITY_DEVICE_ID", deviceId, 3600 * 24 * 365 * 100);
		}
