...
public class CaptchaFilter implements Filter
{
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException
	{
		...
		{
			Boolean captchaSessionToken = (Boolean) session.getAttribute(captchaSessionTokenName);

			if ((captchaSessionToken == null) || Boolean.FALSE.equals(captchaSessionToken))
			{
				logger.debug("captchaSessionToken is null or false : {}", captchaSessionToken);
				chain.doFilter(request, response);
		...
		String captchaParameterValue = request.getParameter(captchaParameterName);
		String captchaSessionValue = (String) session.getAttribute(captchaSessionAttributeName);

		if ((captchaParameterValue != null) && captchaParameterValue.equals(captchaSessionValue))
		{
		...
		else
		{
			...
			session.setAttribute("SPRING_SECURITY_LAST_EXCEPTION", new RuntimeException("验证码不匹配"));
