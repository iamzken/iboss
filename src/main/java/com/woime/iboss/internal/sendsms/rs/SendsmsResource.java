package com.woime.iboss.internal.sendsms.rs;
...
{
	private static Logger logger = LoggerFactory.getLogger(SendsmsResource.class);
	private SendsmsDataService sendsmsDataService;

	@Path("send")
	@POST
	public BaseDTO send(@FormParam("mobile") String mobile, @FormParam("message") String message, @FormParam("configCode") String configCode)
	{
		...
		boolean configExists = sendsmsDataService.checkConfigCodeExists(configCode);
