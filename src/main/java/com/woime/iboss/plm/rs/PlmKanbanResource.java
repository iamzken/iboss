package com.woime.iboss.plm.rs;
...
{
	private static Logger logger = LoggerFactory.getLogger(PlmKanbanResource.class);
	...
	@Produces(MediaType.APPLICATION_JSON)
	public BaseDTO kanbanCreateIssue(@FormParam("name") String name, @FormParam("content") String content, @FormParam("sprintId") Long sprintId, @FormParam("step") String step,
	...
	@Produces(MediaType.APPLICATION_JSON)
	public BaseDTO kanbanUpdateIssue(@FormParam("id") Long id, @FormParam("name") String name, @FormParam("content") String content, @FormParam("step") String step,
			@FormParam("assigneeId") String assigneeId) throws Exception
	{
	...
	@Produces(MediaType.APPLICATION_JSON)
	public BaseDTO kanbanChangeStep(@FormParam("issueId") Long issueId, @FormParam("step") String step) throws Exception
	{
		PlmIssue plmIssue = plmIssueManager.get(issueId);
		PlmIssue oldIssue = new PlmIssue();
		beanMapper.copy(plmIssue, oldIssue);

		PlmStep plmStep = plmStepManager.findUnique("from PlmStep where plmConfig=? and code=?", plmIssue.getPlmSprint().getPlmConfig(), step);
		plmIssue.setStep(step);
