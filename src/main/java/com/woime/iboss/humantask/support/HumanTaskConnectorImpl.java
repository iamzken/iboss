...
import com.woime.iboss.api.humantask.HumanTaskDefinition;
import com.woime.iboss.api.humantask.ParticipantDTO;
import com.woime.iboss.core.mapper.BeanMapper;
import com.woime.iboss.core.page.Page;
import com.woime.iboss.humantask.listener.HumanTaskListener;
...
import com.woime.iboss.spi.process.ProcessTaskDefinition;
...
public class HumanTaskConnectorImpl implements HumanTaskConnector
{
	...
	{
		List<TaskInfo> taskInfos = taskInfoManager.find("from TaskInfo where processInstanceId=? order by createTime asc", processInstanceId);
	...
	public FormDTO findTaskForm(String humanTaskId)
	{
		...
		if (humanTaskDto.getTaskId() != null)
		{
			...
			// .getTaskId());
			com.woime.iboss.spi.humantask.FormDTO taskFormDto = taskDefinitionConnector.findForm(humanTaskDto.getCode(), humanTaskDto.getProcessDefinitionId());

			if (taskFormDto == null)
			{
				logger.info("cannot find form by code : {}, processDefinition : {}", humanTaskDto.getCode(), humanTaskDto.getProcessDefinitionId());
			}
			else
			{
				formDto = new FormDTO();
				...
				List<String> operations = taskDefinitionConnector.findOperations(humanTaskDto.getCode(), humanTaskDto.getProcessDefinitionId());
	 ...
	 * 完成任务.
	 */
	public void completeTask(String humanTaskId, String userId, String action, String comment, Map<String, Object> taskParameters)
	{
	...
	{
		Page page = taskInfoManager.pagedQuery("from TaskInfo where assignee=? and tenantId=? and status='active'", pageNo, pageSize, userId, tenantId);
	...
	{
		Page page = taskInfoManager.pagedQuery("from TaskInfo where assignee=? and tenantId=? and status='complete'", pageNo, pageSize, userId, tenantId);
		List<TaskInfo> taskInfos = (List<TaskInfo>) page.getResult();
		List<HumanTaskDTO> humanTaskDtos = this.convertHumanTaskDtos(taskInfos);
		page.setResult(humanTaskDtos);
	...
	{
		Page page = taskInfoManager.pagedQuery("from TaskInfo where owner=? and tenantId=? and status='active'", pageNo, pageSize, userId, tenantId);
		List<TaskInfo> taskInfos = (List<TaskInfo>) page.getResult();
		List<HumanTaskDTO> humanTaskDtos = this.convertHumanTaskDtos(taskInfos);
		page.setResult(humanTaskDtos);
	...
	{
		String userSql = "select pe.ID as ID from PARTY_ENTITY pe,PARTY_TYPE type" + " where pe.TYPE_ID=type.ID and type.TYPE='1' and pe.REF=?";
