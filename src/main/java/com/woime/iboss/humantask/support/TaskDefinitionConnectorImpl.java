...
import com.woime.iboss.humantask.persistence.domain.TaskConfUser;
import com.woime.iboss.humantask.persistence.domain.TaskDefBase;
import com.woime.iboss.humantask.persistence.domain.TaskDefDeadline;
import com.woime.iboss.humantask.persistence.domain.TaskDefNotification;
import com.woime.iboss.humantask.persistence.domain.TaskDefOperation;
import com.woime.iboss.humantask.persistence.domain.TaskDefUser;
import com.woime.iboss.humantask.persistence.manager.TaskConfUserManager;
import com.woime.iboss.humantask.persistence.manager.TaskDefBaseManager;
import com.woime.iboss.humantask.persistence.manager.TaskDefDeadlineManager;
import com.woime.iboss.humantask.persistence.manager.TaskDefNotificationManager;
import com.woime.iboss.humantask.persistence.manager.TaskDefOperationManager;
import com.woime.iboss.humantask.persistence.manager.TaskDefUserManager;
import com.woime.iboss.spi.humantask.CounterSignDTO;
import com.woime.iboss.spi.humantask.DeadlineDTO;
import com.woime.iboss.spi.humantask.FormDTO;
import com.woime.iboss.spi.humantask.TaskDefinitionConnector;
import com.woime.iboss.spi.humantask.TaskDefinitionDTO;
import com.woime.iboss.spi.humantask.TaskNotificationDTO;
...
public class TaskDefinitionConnectorImpl implements TaskDefinitionConnector
{
	...
	public List<String> findOperations(String taskDefinitionKey, String processDefinitionId)
	{
		...
		List<TaskDefOperation> taskDefOperations = taskDefOperationManager.find(hql, taskDefinitionKey, processDefinitionId);
	...
	public void create(TaskDefinitionDTO taskDefinition)
	{
		...
		TaskDefBase taskDefBase = taskDefBaseManager.findUnique(hql, taskDefinition.getCode(), processDefinitionKey, processDefinitionVersion);
		...
		for (TaskUserDTO taskUser : taskDefinition.getTaskUsers())
		{
			...
			TaskDefUser taskDefUser = taskDefUserManager.findUnique(hqlFindTaskDefUser, taskDefBase, value, type, catalog);
	...
	public void addTaskUser(String taskDefinitionKey, String processDefinitionId, TaskUserDTO taskUser)
	{
		...
		if (taskDefUser != null)
		{
			logger.info("cannot find taskDefUser {} {} {} {}", taskDefBase.getId(), taskUser.getCatalog(), taskUser.getType(), taskUser.getValue());
	 ...
	 * 更新参与者.
	 */
	public void updateTaskUser(String taskDefinitionKey, String processDefinitionId, TaskUserDTO taskUser, String status)
	{
		String hql = "from TaskDefBase where code=? and processDefinitionId=?";
		TaskDefBase taskDefBase = taskDefBaseManager.findUnique(hql, taskDefinitionKey, processDefinitionId);
