...
import com.woime.iboss.api.org.OrgDTO;
import com.woime.iboss.party.PartyConstants;
...
public class PartyOrgConnector implements OrgConnector
{
	...
	public PartyEntity findSuperior(PartyEntity child)
	{
		...
		while (partyEntity != null)
		{
			...
			for (PartyStruct partyStruct : partyEntity.getChildStructs())
			{
				...
				// 如果叶子是岗位，并且这个职位是管理岗位
				if ((childPartyEntity.getPartyType().getType() == PartyConstants.TYPE_POSITION) && this.isAdmin(partyStruct))
				{
					// 就找这个岗位下面的管理者
					superior = this.findAdministrator(childPartyEntity);
				}
				else if ((childPartyEntity.getPartyType().getType() == PartyConstants.TYPE_USER) && this.isAdmin(partyStruct))
				{
	...
	public PartyEntity findUpperDepartment(PartyEntity child)
	{
		...
		for (PartyStruct partyStruct : child.getParentStructs())
		{
			...
			if ((parent.getPartyType().getType() == PartyConstants.TYPE_ORG) && this.isNotAdmin(partyStruct))
			{
