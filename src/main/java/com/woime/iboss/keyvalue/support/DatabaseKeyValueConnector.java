package com.woime.iboss.keyvalue.support;
...
public class DatabaseKeyValueConnector implements KeyValueConnector
{
	...
	public Record findByCode(String code)
	{
		...
		{
			logger.info("根据业务主键无法找到Record记录 : {}", code);
	...
	{
		long totalCount = jdbcTemplate.queryForObject("select count(*) from KV_RECORD WHERE STATUS=? AND USER_ID=? AND TENANT_ID=?", Long.class, status, userId, tenantId);
		List<Map<String, Object>> list = jdbcTemplate.queryForList("SELECT * FROM KV_RECORD WHERE STATUS=? AND USER_ID=? AND TENANT_ID=? limit ?,?", status, userId, tenantId, page.getStart(),
				page.getPageSize());
		List<Record> records = new ArrayList<Record>();
	...
	{
		String sqlRecordInsert = "insert into KV_RECORD(id,name,form_template_code,category,status,ref,create_time,user_id,tenant_id)" + " values(?,?,?,?,?,?,?,?,?)";
		...
		jdbcTemplate.update(sqlRecordInsert, id, name, formTemplateCode, record.getCategory(), record.getStatus(), ref, createTime, userId, tenantId);
		...
		{
			jdbcTemplate.update(sqlProp, idGenerator.generateId(), prop.getCode(), prop.getType(), prop.getValue(), record.getCode());
		}
	}
	...
	public void update(Record record)
	{
		...
		for (Prop prop : record.getProps().values())
		{
			...
			{
				jdbcTemplate.update(sqlPropInsert, idGenerator.generateId(), prop.getCode(), prop.getType(), prop.getValue(), record.getCode());
			}
		}
	}
	...
	public Record copyRecord(Record original, List<String> fields)
	{
		...
		List<Map<String, Object>> list = jdbcTemplate.queryForList("select * from KV_PROP where record_id=?", original.getCode());
