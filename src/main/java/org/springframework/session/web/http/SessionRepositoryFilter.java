 ...
 * {@link org.springframework.session.ExpiringSession}
...
public class SessionRepositoryFilter<S extends ExpiringSession> extends OncePerRequestFilter
{
	...
	/**
	 * Invalid session id (not backed by the session repository) request
	 * attribute name.
	 */
	 ...
	 *
	 * @param httpSessionStrategy
	 *            the {@link MultiHttpSessionStrategy} to use. Cannot be
	 *            null.
	 */
	...
	private final class SessionRepositoryRequestWrapper extends HttpServletRequestWrapper
	{
		...
		/**
		 * Uses the HttpSessionStrategy to write the session id to the response
		 * and
		 * persist the Session.
		 */
		...
		public HttpSessionWrapper getSession(boolean create)
		{
			...
			if (requestedSessionId != null && getAttribute(INVALID_SESSION_ID_ATTR) == null)
			{
				...
				else
				{
					...
					// request.getSession is invoked for the duration of this
					// request
