    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
          ...
          <a href="${tenantPrefix}/user/my-device-active.do?id=${item.id}" class="a-active">信任</a>
		  </c:if>
		  <c:if test="${item.status != 'disabled'}">
          <a href="${tenantPrefix}/user/my-device-disable.do?id=${item.id}" class="a-disable">禁用</a>
		  </c:if>
          <a href="${tenantPrefix}/user/my-device-remove.do?id=${item.id}" class="a-remove">删除</a>
        </td>
      </tr>
      </c:forEach>
    </tbody>
