      ...
      <tr>
	    ...
	    <td>
		  <c:if test="${item.suspended}">
		    挂起
            <a href="bpm/console-activeProcessInstance.do?processInstanceId=${item.id}">(激活)</a>
		  </c:if>
		  <c:if test="${not item.suspended}">
		    激活
            <a href="bpm/console-suspendProcessInstance.do?processInstanceId=${item.id}">(挂起)</a>
		  </c:if>
		</td>
        <td>
          <a href="bpm/console-removeProcessInstance.do?processInstanceId=${item.id}">删除</a>
          <a href="bpm/console-migrateInput.do?processInstanceId=${item.id}">迁移</a>
          <a href="bpm/workspace-viewHistory.do?processInstanceId=${item.id}">历史</a>
          ...
          <a href="bpm/console-deleteProcessInstance.do?id=${item.id}">删除（包含历史）</a>
        </td>
      </tr>
      </c:forEach>
    </tbody>
