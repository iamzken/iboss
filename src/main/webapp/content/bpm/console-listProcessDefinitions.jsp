...
--%>
      <div style="margin-bottom: 20px;">
	    <div class="pull-left btn-group" role="group">
		  <button class="btn btn-default a-insert" onclick="location.href='bpm/console-process-input.do'">上传</button>
		<%--
      ...
      <tr>
	    ...
	    <td>
		  <c:if test="${item.suspended}">
		    挂起
            <a href="bpm/console-activeProcessDefinition.do?processDefinitionId=${item.id}">(激活)</a>
		  </c:if>
		  <c:if test="${not item.suspended}">
		    激活
            <a href="bpm/console-suspendProcessDefinition.do?processDefinitionId=${item.id}">(挂起)</a>
		  </c:if>
		</td>
        <td>
          <a href="bpm/console-graphProcessDefinition.do?processDefinitionId=${item.id}" target="_blank">流程图</a>
          <a href="bpm/console-viewXml.do?processDefinitionId=${item.id}" target="_blank">查看XML</a>
          <a href="${tenantPrefix}/widgets/diagram-viewer/index.html?processDefinitionId=${item.id}" target="_blank">diagram-viewer</a>
          <a href="bpm/console-beforeUpdateProcess.do?processDefinitionId=${item.id}">修改</a>
        </td>
      </tr>
      </c:forEach>
    </tbody>
