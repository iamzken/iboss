  ...
  <table id="demoGrid" class="table table-hover">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
          <a href="bpm/workspace-prepareCompleteTask.do?taskId=${item.id}">完成</a>
		  <c:if test="${assignee==null}">
          <a href="bpm/workspace-claimTask.do?taskId=${item.id}">认领</a>
		  </c:if>
		  <c:if test="${owner==null}">
          <a href="bpm/workspace-delegateTask.do?taskId=${item.id}">代理</a>
		  </c:if>
		  <c:if test="${assignee != null && owner != null}">
          <a href="bpm/workspace-resolveTask.do?taskId=${item.id}">处理</a>
		  </c:if>
          <a href="bpm/workspace-viewHistory.do?processInstanceId=${item.processInstanceId}">历史</a>
          <a href="bpm/console-prepareJump.do?executionId=${item.executionId}">自由跳转</a>
          <a href="bpm/console-addSubTaskInput.do?taskId=${item.id}">添加子任务</a>
        </td>
      </tr>
      </c:forEach>
    </tbody>
