  ...
  <table id="pimRemindGrid" class="table table-hover">
    ...
    <tbody>
      ...
      <tr>
	    ...
	    <td>${item.id}</td>
		-->
	    <td><a href="bpm/workspace-viewHistory.do?processInstanceId=${item.id}">${item.name}</a></td>
	    <td><tags:processName processDefinitionId="${item.processDefinitionId}"/></td>
	    <td><fmt:formatDate value="${item.startTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
	    <td>运行中</td>
        <td>
          <a href="bpm/workspace-endProcessInstance.do?processInstanceId=${item.id}">终止</a>
          <a href="bpm/workspace-remind.do?processInstanceId=${item.id}&userId=&comment=">催办</a>
          <a href="bpm/workspace-skip.do?processInstanceId=${item.id}&userId=&comment=">跳过</a>
		  <!--
          <a href="workspace-withdraw.do?processInstanceId=${item.id}">撤销</a>
		  -->
          <a href="bpm/workspace-viewHistory.do?processInstanceId=${item.id}">详情</a>
        </td>
      </tr>
      </c:forEach>
    </tbody>
